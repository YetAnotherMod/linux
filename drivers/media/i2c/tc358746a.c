...
static struct v4l2_subdev *tc358746a_get_remote_sd(struct v4l2_subdev *sd)
{
	...
	r_pad = media_entity_remote_pad(&tc358746a->pad[TC358746A_IN_PAD]);
...
#define TC358746_MK_PAD_OP(_op, _type)                                    \
	...
	{                                                                 \
		...
		if (!remote)                                              \
			return -ENODEV;                                   \
                                                                          \
		return v4l2_subdev_call(remote, pad, _op, cfg, s);         \
	}
			     ...
			     struct v4l2_subdev_format *s)
{
	...
	if (!remote)
		return -ENODEV;
	ret = v4l2_subdev_call(remote, pad, get_fmt, cfg, s);
	if (ret)
		return ret;
	pixcode = tc358746a->force_subdev_pixcode;
	if (pixcode) {
				    ...
				    struct v4l2_subdev_mbus_code_enum *code)
{
	...
	if (!remote)
		return -ENODEV;
	ret = v4l2_subdev_call(remote, pad, enum_mbus_code, cfg, code);
	if (ret)
		return ret;
	pixcode = tc358746a->force_subdev_pixcode;
	if (pixcode) {
...
tc358746a_get_register(struct v4l2_subdev *sd, struct v4l2_dbg_register *reg)
{
	int ret;
	u16 val;
	reg->size = 2;
...
static int __maybe_unused 
tc358746a_set_register(struct v4l2_subdev *sd, const struct v4l2_dbg_register *reg)
{
...
#define IS_BETWEEN(_f, _min, _max) ((_f >= _min) && (_f <= _max))
	/* Check Ref frequency */
	if (!IS_BETWEEN(tc358746a->refclk, TC358746A_MIN_REFCLK,
...
ppi_ok:
	...
	for (frs = 0 ; frs < 4 ; frs++) {
		for (prd = 0 ; prd < 16 ; prd++) {
			for (fbd = 0 ; fbd < 512 ; fbd++) {
				...
				unsigned long error;
				pll_clk_test = tc358746a->refclk * (fbd + 1) /
					       (prd + 1) / (1 << frs);
	...
	/* Update pclk */
	tc358746a->pclk = tc358746a->refclk * (pllctl0.pll_fbd + 1) /
			  (pllctl0.pll_prd + 1) /
			  (1 << pllctl1.pll_frs) /
			  div[clkctl.sclkdiv];
	/* MCLK configuration */
...
static int tc358746a_calibrate(struct v4l2_subdev *sd)
{
	...
	error_wait = tc358746a->calibration_delay_ms;
	if (error_wait == 0) {
		/* Default to 30ms */
		error_wait = 30;
	}
...
static int tc358746a_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	if (enable == 0) {
		...
		if (tc358746a->set_power)
			tc358746a->set_power(TC358746A_POWER_OFF);
		return ret;
	}
	 ...
	 */
	if (tc358746a->set_power)
		tc358746a->set_power(TC358746A_POWER_ON);
	/* Software reset */
	...
	/* ConfCtl */
	confctl.datalane  = tc358746a->lanes - 1,
	confctl.auto_incr = 1,
	confctl.pclkp     = 0,
	confctl.hsyncp    = 0,
	confctl.vsyncp    = 0,
...
static int tc358746a_detect_chip(struct v4l2_subdev *sd)
{
	...
	u16                             chip_id = 0;
	if (tc358746a->set_power) {
		ret = tc358746a->set_power(TC358746A_POWER_ON);
		if (ret) {
	...
	if (chip_id == TC358746A_CHIPID ||
		...
		v4l2_info(sd, "Found tc358746a chip\n");
		return 0;
	}
...
static int tc358746a_probe(struct i2c_client *client)
{
	...
	tc358746a = devm_kzalloc(dev, sizeof(*tc358746a), GFP_KERNEL);
	if (!tc358746a)
		return -ENOMEM;

	//Need set from DT
	tc358746a->set_power = NULL;
	tc358746a->refclk = 27000000;
	tc358746a->lanes = 2;
	tc358746a->force_subdev_pixcode = 0;
	tc358746a->calibration_delay_ms = 0;
	tc358746a->phytimdly = 0;


	if (tc358746a->refclk == 0) {
		dev_err(dev, "refclk frequency is not specified\n");
		return -EINVAL;
	}
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_SMBUS_BYTE_DATA)) {
		dev_err(dev, "i2c not available\n");
		return -ENODEV;
	}
	...
	ret = media_entity_pads_init(&sd->entity, TC358746A_NUM_PADS, tc358746a->pad);
	if (ret < 0) {
...
static const struct of_device_id tc358746a_dt_ids[] = {
	{ .compatible = "toshiba,tc358746a" },
	{ /* sentinel */ }
};
MODULE_DEVICE_TABLE(of, tc358746a_dt_ids);

static struct i2c_driver tc358746a_i2c_driver = {
	.driver = {
		.name  = "tc358746a",
		.of_match_table = tc358746a_dt_ids,
	},
	.probe_new = tc358746a_probe,
	.remove = tc358746a_remove,
};

module_i2c_driver(tc358746a_i2c_driver);
