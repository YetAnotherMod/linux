...
static struct pci_ops rcm_cdns_pcie_host_ops = {
	.map_bus	= rcm_cdns_pci_map_bus,
...
static int cdns_pcie_host_init_root_port(struct cdns_pcie_rc *rc)
{
	...
	if (rc->vendor_id != 0xffff)
		rcm_cdns_pcie_rp_writew(pcie, PCI_VENDOR_ID, rc->vendor_id);
	if (rc->device_id != 0xffff)
		rcm_cdns_pcie_rp_writew(pcie, PCI_DEVICE_ID, rc->device_id);

	rcm_cdns_pcie_rp_writeb(pcie, PCI_CLASS_REVISION, 0);
	rcm_cdns_pcie_rp_writeb(pcie, PCI_CLASS_PROG, 0);
	rcm_cdns_pcie_rp_writew(pcie, PCI_CLASS_DEVICE, PCI_CLASS_BRIDGE_PCI);

	return 0;
}

static int rcm_cdns_pcie_host_init_address_translation(struct cdns_pcie_rc *rc)
{
	struct cdns_pcie *pcie = &rc->pcie;
	...
	u64 sz = 1ULL << fls64(resource_size(cfg_res) - 1);
	int nbits = ilog2(sz);
	...
	for_each_of_pci_range(&parser, &range) {
		...
		rcm_cdns_pcie_set_outbound_region(pcie, 0, r, is_io,
		                                  range.cpu_addr,
		                                  range.pci_addr,
		                                  range.size);
		r++;
...
static int rcm_cdns_pcie_host_init(struct device *dev,
                                   ...
                                   struct cdns_pcie_rc *rc)
{
	...
	err = rcm_cdns_pcie_host_init_address_translation(rc);
	if (err)
		goto err_out;
...
int rcm_cdns_pcie_host_setup(struct cdns_pcie_rc *rc)
{
	...
	ret = rcm_cdns_pcie_host_init(dev, &resources, rc);
	if (ret)
		goto err_init;
	...
	bridge->ops = &rcm_cdns_pcie_host_ops;
